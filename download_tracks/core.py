import datetime as dt, hashlib, re, time, typing
...
from .mime_utils import MimeType, guess_mime_type

UNSAFE_PATH_CLEAR_RE = re.compile(r"[/\\]+")
SAFE_PATH_CLEAR_RE = re.compile(r"([^\w\-\'() ]|^\s+|\s+$)")

DEFAULT_PATH_PATTERN = Path("#album-artist", "#album", "#title")
...
def set_tags(
    ...
    if isinstance(tag, MP3):
        ...
        if album_cover:
            # type=3 is COVER_FRONT (standard for music players and Telegram)
            tag["APIC"] = APIC(
                encoding=3,
                mime=album_cover.mime_type.value,
                type=3,  # COVER_FRONT
                desc="",  # Empty description for compatibility
        ...
        if album_cover:
            ...
            # Telegram reads the covr atom - ensure it's properly formatted
            tag["covr"] = [MP4Cover(album_cover.data, imageformat=mp4_image_format)]
            # Also add as artwork metadata for compatibility
            tag["rtng"] = [1]  # Rating (helps some players/apps recognize metadata)
...
def download_track(
    ...
    need_cover = bool(embed_cover or write_cover_file)
    if need_cover and track.cover_uri is not None:
